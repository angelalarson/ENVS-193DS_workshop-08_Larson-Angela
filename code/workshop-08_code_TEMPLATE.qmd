...
drought_exp_clean <- drought_exp |> # starting with drought_exp object
  ...
  clean_names() |> 
  # adding new column for scientific name
  ...
  mutate(species_name = as_factor(species_name),
         species_name = fct_relevel(species_name,
                                    ...
                                    "Grindelia camporum")) |> 
  # making sure that water_treatment and reordering levels
...
```{r model-diagnostics}
par(mfrow = c(2, 2)) # displays plots in a 2 x 2 grid
plot(model1)
plot(model2)
plot(model3)
plot(model4)
...
```{r model-summary}
summary(model4)
# reference level for water_treatment: drought stresses (doesnt show up in the summary)
...
model4_predictions <- ggpredict(
  ...
  # rename columns (treating this like a df)
